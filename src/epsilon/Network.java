...
public final class Network implements Serializable {
  ...
  public Network(Vector config, ActivationFunction[] activationFunction, Error errorType, InitChoice initChoice) {
    ...
    for (int i = 0; i < config.length() - 1; i++) {
      ...
      switch (initChoice) {
        case ZERO:
          layer.fillZeros((int) config.get(i), (int) config.get(i + 1));
          break;
        case RANDOM:
          layer.fillRandom((int) config.get(i), (int) config.get(i + 1));
          break;
        case GAUSSIAN:
          layer.fillGaussian((int) config.get(i), (int) config.get(i + 1));
          break;
        default:
  ...
  public void train(double[][] input, double[][] target, int epoch, double alpha, Optimizer optimizer, int BATCH_SIZE) {
    ...
    { // Give approximate time prediction
